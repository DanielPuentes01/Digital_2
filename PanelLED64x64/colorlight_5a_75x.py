...
from gateware.screen_controller64x64 import ScreenController64x64

# CRG ----------------------------------------------------------------------------------------------

class _CRG(LiteXModule):
    def __init__(self, platform, sys_clk_freq, with_hub75_25Mhz=False , use_internal_osc=False, with_usb_pll=False, sdram_rate="1:1",simulation=False):
        self.rst    = Signal()
...
#            assert use_internal_osc, "You cannot use the 25MHz clock as system clock since it is provided by the Ethernet PHY and will stop during PHY reset."
        ...
        # Ethernet / Etherbone ---------------------------------------------------------------------
        if with_etherbone and not simulation:
